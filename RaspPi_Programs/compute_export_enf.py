...
folder_database_id = '1ZAFMQeStQJEgWT4JEiVeSRIvZOwHNxf5' #upload to database folder
dev = 'Dev1'
...
def scan_folders(target, file_name):
    ...
    if create_folder_flag == 0:    # no ->> create folder
        new_folder_id = create_folder(folder_database_id, dev + '_Power_ENF_'+target)
        upload_file(new_folder_id, file_name)
...
def main():
    ...
    while(True):
        ...
        if (not list_audio_files): 
            print('Folder empty')
            time.sleep(1800) # sleep for some time
            continue
        ...
        for filename in list_ENF_files:
            ...
            scan_folders(target,filename) # scan and upload files to proper folder
